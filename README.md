...
- 🎨 **现代UI/UX**：精美渐变界面，流畅动画
- 📄 **SRT字幕生成与嵌入**：自动生成翻译后的SRT字幕文件并嵌入视频（软字幕轨道，可在播放器中开关）
...
**修复：**
- 🐛 **修复翻译重复问题**：改用 JSON 格式输出，避免解析错位和重复翻译
  - 提示词要求 AI 返回 JSON 数组
  - 解析响应时提取 JSON 数组，带降级处理
  - 验证翻译结果数量与输入一致
- 🐛 **修复字幕未嵌入问题**：自动将 SRT 字幕嵌入视频为软字幕轨道
  - `mergeVideoAudio` 新增可选参数 `srtPath`
  - 使用 FFmpeg 将 `.srt` 字幕嵌入为软字幕轨道
  - 播放时可在字幕开关中开启/关闭显示
- 🐛 **修复 srtPath 变量作用域问题**
  - 将 `srtPath` 声明提升到 `processDubbingJob` 函数顶层
  - 避免 `try` 块内 `const` 声明导致的作用域隔离

**优化：**
- ⚡ **翻译可靠性提升**：JSON 格式解析，避免逐行文本解析的错误
- ⚡ **字幕体验提升**：输出视频直接包含字幕轨道，无需额外加载
